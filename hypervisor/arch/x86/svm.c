...
static int vmcb_setup(struct per_cpu *cpu_data)
{
	...
	vmcb->cr0 = cpu_data->linux_cr0 & ~SVM_CR0_CLEARED_BITS;
...
int vcpu_init(struct per_cpu *cpu_data)
{
	...
	write_cr0((read_cr0() & X86_CR0_RESERVED) | X86_CR0_HOST_STATE);
	write_cr4((read_cr4() & X86_CR4_RESERVED) | X86_CR4_HOST_STATE);
